import { Container, Point, Event } from 'createjs-module';
import { S } from './basic-intfs';
export class ScaleEvent extends Event {
    constructor(type, scale, scaleNdx) {
...
export class ScaleableContainer extends Container {
    ...
    constructor(parent, params) {
        ...
        this.scaleNdx = 0;
        this.scaleMin = -3; // 0.49990*zero
        this.scaleMax = 30; // 1025.92*zero
        this.scaleBase = 1.26;
        this.scaleZero = 1; // scale to fit TerrainMap [640/1024 = .625]
        this.scaleLimit = 1000;
        this.invScale = 1.0;
        this._unscaled = new Array(); // Set<DisplayObject>
    ...
    */
    makeZoomable(params = {}) {
        ...
        let di = 0; // accumulate scroll increments
        let mouseWheelHandler = ((e) => {
            let pmx = stage.mouseX / stage.scaleX;
            let pmy = stage.mouseY / stage.scaleY;
            let p = new Point(pmx, pmy);
     ...
     */
    findIndex(initScale, set = false) {
        let min = this.scaleMin, max = this.scaleMax, base = this.scaleBase, zero = this.scaleZero;
        let initDist = 99999, initIndex = 1;
        for (let i = min; i <= max; i++) {
            let s = zero * Math.pow(base, i); // i=0 -> s=zero
            // if s>1 try round to int
            if (Math.abs(s - Math.round(s)) < .08 * s)
                s = Math.round(s); // use integral scale when close
            ...
            if (set)
                this.scaleAry[i] = Math.min(s, this.scaleLimit);
        }
        return initIndex;
    }
    /** set scaleIndex & return associated scale factor */
    getScale(ndx = this.scaleNdx) {
     ...
     */
    scaleContainer(di, p) {
        let os = this.getScale(); // current -> old / original scale
        let ns = this.incScale(di);
        if (di == 0) {
            os = 0;
            ns = this.getScale(this.initIndex);
        }
        this.scaleInternal(os, ns, p);
    }
    ...
    removeUnscaled(dobj) {
        let ndx = this._unscaled.indexOf(dobj);
        if (ndx < 0)
            return;
...
//# sourceMappingURL=scaleable-container.js.map