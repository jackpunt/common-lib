import { Container, Event, Rectangle, Shape, Stage, Text } from "createjs-module";
...
export class DropdownButton extends Container {
     ...
     */
    constructor(text, w, h, r, click, style) {
        super();
        // Public Members
        this.style = Obj.fromEntriesOf(DropdownButton.defaultStyle);
        if (!!style)
            this.style = style;
        // Public Methods
        this.pressed = false;
        ...
        // Shape Events
        this.shape.addEventListener("pressmove", (e) => {
            e.stopPropagation(); // do not 'drag' when pressing on menu item
            _self.pressed = true;
            this.render();
        });
        this.shape.addEventListener("pressup", (e) => {
            _self.pressed = false;
            this.render();
        });
        this.shape.addEventListener("mouseover", (e) => {
            //e.stopPropagation()
            _self.hover = true;
            this.render();
        });
        this.shape.addEventListener("mouseout", (e) => {
            ;
...
export class DropdownChoice extends Container {
    ...
    constructor(items, item_w, item_h, style) {
        ...
        let _itemclick = (item) => { this.dropdown(false); this.select(item); };
    ...
    onItemChanged(f) { this._itemChanged = f; }
    ...
    dropdown(expand) {
        if (expand)
            expand = !this._expand;
        if (!expand)
            this.stage.dispatchEvent(new Event("DropdownCollapse", true, true));
    ...
    enable() {
        ...
        this.stage.on("DropdownCollapse", (e) => {
            //this.dropdown(false)
            //this.stage.update()
        });
    }
    ...
    static maxItemWidth(items, font_h, fontName) {
        let w = 0;
        items.forEach(item => w = Math.max(w, new Text(item.text, F.fontSpec(font_h, fontName)).getMeasuredWidth()));
        return w;
    }
    // Public Members
    /**
    ...
    select(item) {
        if (item == undefined)
            item = this._selected; // "reselect" same item [no-op]
